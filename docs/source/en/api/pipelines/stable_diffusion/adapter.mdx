...
cond_keypose = load_image("https://huggingface.co/RzZ/sd-v1-4-adapter-keypose-depth/resolve/main/sample_input_keypose.png")
...
# make sure we only take the RGB channels if input is a RGBA image
cond_keypose = cond_keypose[..., :3]
cond_depth = cond_depth[..., :3]
...
2. There exist a special variation of T2I-Adapter, Style-Adapter, that first make use of CLIP vision encoder to process *control image* first then adapter.
